...
[^sklearn-critique]: Lately, it has become fashionable to bash `scikit-learn`. Bjarne Stroustroup has said about programming languages something along the lines of "there are languages that everyone complains about and languages that no one uses". The same can be said about libraries (or any other tool&mdash;it's a nice pearl of distilled wisdom).