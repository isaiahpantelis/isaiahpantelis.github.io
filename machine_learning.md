...
- The "optimal" degree estimated with cross validation is 14. This cannot be seen in the plot (the lines in the top plot appear flat from degree 12 onwards) because it beats the average test error of the higher degrees at the fourth decimal place.
...
For this example, I generated data on $$[-2,2]$$ and kept $$[-2,-1]\bigcup[1,2]$$ as validation set. You can, of course, select a *random* subset as the validation set, but then the results will look quite different. The situation here is slightly different than the standard textbook example where the fitted polynomial passes through all the training data points and oscillates in-between, Lagrange style. Here, there are thousands of training data points and the degree was only allowed to go up to $$25$$. The hard and fast rule is "know what you're doing". The textbook setup demonstrates what happens when the model has too many "degrees of freedom" relative to the size of the training set,[^dof-overfitting] whereas the present setup is meant to show how poorly the model generalises (although there are 500 training data points in $$[-1,1]$$ and the polynomial degrees go up to 25).
...
[^dof-overfitting]: In the era of neural networks, it is now well known that having a large number of free parameters to calibrate during training is **not** a sufficient condition for overfitting.
