...
Although the newly formulated question about *learning* the data generation mechanism (as oppposed to finding a polynomial that passes through some points) has not been precisely defined, it is in a sense falsifiable and, therefore, worth considering further. So let's pretend we're in a toxic relationship with the Chebyshec polynomials: we tell them one thing and we mean another. Initially, we had asked of them to approximate the function $$f$$ on the interval $$I=[-1,1]$$, but now we expect them to behave like the function $$f$$ also **outside** the interval $$I$$. Extrapolation may seem like an intuitively appealing concept of "learning" (if you are learning the function, then you behave like the function no matter what, right?), but, also, intellectually dishonest in this case. Because, the domain of a function is part of the function's definition&mdash;there are no two ways about it&mdash;and we had asked for an approximation on $$[-1,1]$$ specifically. This is how the Chebyshev interpolant, built out of $$24$$ nodes, behaves on $$J\triangleq[-1.1,1.1]$$:
